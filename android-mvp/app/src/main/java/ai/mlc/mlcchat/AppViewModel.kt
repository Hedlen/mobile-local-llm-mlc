...
class AppViewModel(application: Application) : AndroidViewModel(application) {
    ...
    inner class ChatState {
        ...
        private var modelChatState = mutableStateOf(ModelChatState.Ready)
            @Synchronized get
            @Synchronized set
        private val client = LocalLlmClient().apply { initialize() }
        private var historyMessages = mutableListOf<LocalLlmMessage>()
        ...
        private fun mainResetChat() {
            ...
            executorService.submit {
                callBackend { runBlocking { client.reset() } }
                historyMessages = mutableListOf<LocalLlmMessage>()
                viewModelScope.launch {
                    clearHistory()
                    switchToReady()
                }
        ...
        private fun mainTerminateChat(callback: () -> Unit) {
            executorService.submit {
                callBackend { runBlocking { client.unload() } }
                viewModelScope.launch {
                    clearHistory()
                    switchToReady()
                    callback()
        ...
        private fun mainReloadChat(modelConfig: ModelConfig, modelPath: String) {
            ...
            executorService.submit {
                ...
                if (!callBackend {
                        runBlocking {
                            client.load(modelConfig.modelId, modelPath, modelConfig.modelLib)
                        }
                    }) return@submit
                viewModelScope.launch {
        ...
        fun requestGenerate(prompt: String, activity: Activity) {
            ...
            if (imageUri != null) {
                ...
                content = "$prompt\n[Image selected; multimodal input is outside the text-only MVP SDK.]"
                imageUri = null
            }
                    ...
                    try {
                        client.stream(
                            ...
                            when (event) {
                                ...
                                is LocalLlmEvent.Usage -> {
                                    val prefill = event.usage.prefillTokensPerSecond
                                    val decode = event.usage.decodeTokensPerSecond
                                    report.value = listOfNotNull(
                                        prefill?.let { "prefill: ${String.format("%.1f", it)} tok/s" },
                                        decode?.let { "decode: ${String.format("%.1f", it)} tok/s" },
                                    ).joinToString(", ")
                                }
                                is LocalLlmEvent.Completed -> {
                                    finishReasonLength = event.finishReason == LocalLlmFinishReason.LENGTH
                                }
                                is LocalLlmEvent.Cancelled,
                                is LocalLlmEvent.Started -> Unit
                            }
                        }
                    } catch (error: Exception) {
                        callBackend { throw error }
                    }
