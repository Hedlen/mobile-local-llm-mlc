...
fun GameView(navController: NavController, appViewModel: AppViewModel, activity: Activity) {
    ...
    Scaffold(topBar = { TopAppBar(title = { Text("五子棋 · 本地 AI 助手") }) }) { padding ->
        LazyColumn(Modifier.padding(padding).padding(12.dp), verticalArrangement = Arrangement.spacedBy(10.dp)) {
            ...
            item { Row(horizontalArrangement = Arrangement.spacedBy(8.dp)) { Button(enabled = moves.isNotEmpty() && chat.chatable(), onClick = { chat.requestGenerate("你是五子棋教练。玩家画像：$profile。棋谱：${moves.joinToString(",")}。给出下一手建议、理由和一句鼓励。", activity) }) { Text("分析下一步") }; Button(enabled = moves.isNotEmpty() && chat.chatable(), onClick = { chat.requestGenerate("根据玩家画像‘$profile’和五子棋棋谱‘${moves.joinToString(",")}’，写 80 字复盘故事，并给两条训练建议。", activity) }) { Text("复盘讲故事") } } }
...
private fun GomokuBoard(board: List<Int>, modifier: Modifier, onTap: (Int) -> Unit) {
    Canvas(modifier.background(Color(0xffd9a85b)).pointerInput(Unit) { detectTapGestures { p -> val cell = size.width / (N - 1); val c = (p.x / cell).toInt().coerceIn(0, N - 1); val r = (p.y / cell).toInt().coerceIn(0, N - 1); onTap(r * N + c) } }) {
