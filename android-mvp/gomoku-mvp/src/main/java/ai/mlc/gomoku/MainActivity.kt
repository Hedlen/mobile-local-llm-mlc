...
@Composable fun GomokuApp() {
    ...
    var analyze by remember { mutableStateOf(true) }; var profile by remember { mutableStateOf("稳健学习型") }
    var advice by remember { mutableStateOf("实时推荐由棋局引擎生成；点击“本地 AI 复盘”可调用 Qwen2.5 1.5B。") }
    ...
    val context = LocalContext.current; val service = remember { GomokuModelService(context) }; val scope = rememberCoroutineScope(); val client = remember { LocalLlmClient().apply { initialize() } }
    ...
    fun review() {
        if (!modelReady || reviewing) return
        ...
        val moveCount = position.count { it != 0 }; val capturedWinner = winner; val capturedProfile = profile
        reviewing = true; advice = "本地 Qwen2.5 1.5B 正在复盘；棋盘仍可继续操作。"
            ...
            val result = withContext(Dispatchers.Default) {
                runCatching {
                    ...
                    client.stream(LocalLlmRequest(service.modelId, listOf(
                        LocalLlmMessage(LocalLlmRole.SYSTEM, "你是五子棋复盘教练。用不超过三句中文，结合给定局面事实给出具体战术复盘和下一局训练建议。不得虚构坐标、胜负或棋形。"),
                        LocalLlmMessage(LocalLlmRole.USER, "玩家风格：$capturedProfile；已落子数：$moveCount；局面事实：$insight；赛果：${resultName(capturedWinner)}。"),
                    ), maxOutputTokens = 40, temperature = 0.65f, topP = 0.9f)).collect { event -> if (event is LocalLlmEvent.Delta) output += event.text }
                    GameReview.finish(output, capturedProfile, moveCount, insight, capturedWinner)
                }.getOrElse { GameReview.fallback(capturedProfile, moveCount, insight, capturedWinner) }
            }
    ...
    Scaffold(topBar = { TopAppBar(title = { Text("五子棋 · 本地 AI 助手") }) }) { padding -> Column(Modifier.padding(padding).verticalScroll(rememberScrollState()).padding(12.dp), verticalArrangement = Arrangement.spacedBy(10.dp)) {
        ...
        OutlinedTextField(profile, { profile = it }, label = { Text("玩家风格") }, modifier = Modifier.fillMaxWidth()); Text(status)
        ...
        Button(onClick = ::review, enabled = modelReady && !reviewing, modifier = Modifier.fillMaxWidth()) { Text(if (reviewing) "本地 AI 复盘中…" else "本地 AI 复盘（Qwen2.5 1.5B）") }
