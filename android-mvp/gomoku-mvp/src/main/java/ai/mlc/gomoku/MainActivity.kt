...
@Composable fun GomokuApp() {
    ...
    fun tap(i: Int) { if (winner != 0 || board[i] != 0) return; val b = board.toMutableList(); b[i] = 1; val ai = b.indices.filter { b[it] == 0 }.minByOrNull { kotlin.math.abs(it / N - 7) + kotlin.math.abs(it % N - 7) }; if (ai != null) b[ai] = 2; board = b; if (analyze) { advice = "正在分析当前局面…"; scope.launch { runCatching { if (client.state != LocalLlmClientState.READY) client.load(service.modelId, service.directory().absolutePath, service.modelLib); var out = ""; client.stream(LocalLlmRequest(service.modelId, listOf(LocalLlmMessage(LocalLlmRole.SYSTEM, "你是五子棋教练，只输出具体行列落点和攻防理由。"), LocalLlmMessage(LocalLlmRole.USER, "玩家风格=$profile；当前棋盘=${b.joinToString()};刚落子=${i / N + 1},${i % N + 1}")), maxOutputTokens = 96, temperature = .2f)).collect { e -> if (e is LocalLlmEvent.Delta) out += e.text }; advice = out }.onFailure { advice = "本地模型暂不可用：${it.message}" } } } }
